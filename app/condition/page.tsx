...
const CONDITION_OPTIONS = ['Rusak Ringan', 'Rusak Berat', 'Diputihkan', 'Hilang'];
...
export default function ConditionPage() {
  ...
  const [fromCondition, setFromCondition] = useState('Baik');
  const [toCondition, setToCondition] = useState('Rusak Ringan');
  ...
  function getEventType() {
    if (toCondition === 'Diputihkan') return 'writeoff';
    if (toCondition === 'Hilang') return 'lost';
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      alert('Gagal mencatat perubahan');
      setLoading(false);
    }
  }
  ...
  return (
    <div className="max-w-xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Perubahan Kondisi / Write-off</h1>
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow p-6 space-y-4">
        ...
        <div>
          ...
          <select value={locationId} onChange={e => setLocationId(e.target.value)} required className="w-full border rounded-lg px-3 py-2 text-sm">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium mb-1">Dari Kondisi</label>
            <select value={fromCondition} onChange={e => setFromCondition(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm">
              <option>Baik</option>
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Ke Kondisi</label>
            <select value={toCondition} onChange={e => setToCondition(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm">
        ...
        <div>
          <label className="block text-sm font-medium mb-1">Alasan / Catatan</label>
        ...
        <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white px-6 py-2 rounded-lg text-sm hover:bg-blue-700 disabled:opacity-50">
          {loading ? 'Menyimpan...' : 'Catat Perubahan'}
        </button>
      </form>
    </div>
  );
